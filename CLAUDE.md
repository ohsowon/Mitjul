...
### 작업 방식 (중요)
- **Claude가 직접 파일을 만들거나 수정하거나 명령을 실행하지 않는다.** 개발자가 직접 작성·실행한다.
- 작성할 코드·파일은 **전체 내용을 텍스트(코드블록)로** 제시한다. 파일 경로도 함께 명시한다.
- 실행·빌드·테스트·git 등 모든 절차는 **단계별 안내(설명)** 로만 제공한다. 개발자가 보고 직접 따라 한다.
- 코드에는 **주석을 넣지 않는다.** 설명이 필요하면 채팅 본문으로 전달한다(이름·구조로 의도 표현).
- 마일스톤 단위로 나눠 제안하고, 왜 그렇게 설계했는지 학습용 설명을 함께 준다.
